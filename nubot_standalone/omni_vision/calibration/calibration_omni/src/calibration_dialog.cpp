    ...
    ui(new Ui::Dialog)
{
    ...
    for(std::size_t b =0 ;b< 64 ;b++)
        for(std::size_t g=0 ;g < 64 ;g++)
             ...
             {
                 color_space_bgr_[0].at<cv::Vec3b>(0,idx)=cv::Vec3b(4*b,4*g,4*r); //y u v;
                 CTable_bgr_[idx]=3;
                 idx++;
              }
    ...
    calibration_result_dir_="/home/nubot/ros_workspace/src/nubot_standalone/omni_vision/calibration/";
    ui->setupUi(this);
}
...
Dialog::on_loadImage_Btn_clicked()
{
    ...
    QString info=FileDialog.getOpenFileName(this,tr("Open　Image"), "/home/nubot/ros_workspace/src/nubot_standalone/omni_vision/calibration/",
...
void Dialog::on_YUV_RadioBtn_clicked()
{
    ...
    for(std::size_t i =0 ; i < orignal_img_[2].rows; i++ )
    {
        for(std::size_t j= 0 ; j < orignal_img_[2].cols; j++)
        {
            ...
            color_rgb=orignal_img_[0].at<cv::Vec3b>(i,j);
            segment_img_[2].at<cv::Vec3b>(255-int(color_yuv[2]),int(color_yuv[1]))=color_rgb;
        }
    }
    cv::Mat show_yuv;
...
Dialog::update_calibration_result()
{
    for(std::size_t i =0 ;i < calib_interim_result_.size();i++)
    {
        ...
        for(std::size_t i=0; i < pts.size() ; i++)
        {
            QPoint pt= QPoint(pts[i].x()/scale_factor_,256-int(pts[i].y()/scale_factor_));
...
{
    QFileDialog FileDialog;
    QString path=FileDialog.getExistingDirectory(this, tr("Open Calibration Files"),  "/home/nubot/ros_workspace/src/nubot_standalone/omni_vision/calibration/",
...
void Dialog::load_interim_result()
{
    ...
    for(std::size_t i =0 ;i < calib_final_result_.size();i++)
    {
        ...
        for(std::size_t i=0; i < pts.size() ; i++)
        {
            QPoint pt= QPoint(pts[i].x()*scale_factor_,(256-pts[i].y())*scale_factor_);
            pts_tmp.push_back(pt);
        }
...
{
    static nubot::Omni_Image omni_img;
    cv::Mat image;
    cv::cvtColor(orignal_img_[0],image,CV_RGB2BGR);
    if(orignal_img_[0].channels()!=3)
        return ;
    omni_img.set_bgr_image(image);
    omni_img.set_hsv_image(orignal_img_[1]);
    omni_img.set_yuv_image(orignal_img_[2]);
    ...
    omni_img.set_big_roi(circle_tmp);
    circle_tmp.radius_=small_radius_;
    omni_img.set_small_roi(circle_tmp);
    omni_img.set_height(orignal_img_[0].rows);
    omni_img.set_width(orignal_img_[0].cols);
    ...
    std::ofstream whites_save(str.c_str());
    for(std::size_t i = 0; i < whites.img_white_.size();i++)
         whites_save<<whites.img_white_[i].x_<<" "<<whites.img_white_[i].y_<<" \n";
     whites_save.close();
}
