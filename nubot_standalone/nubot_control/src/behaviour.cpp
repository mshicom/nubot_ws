...
Behaviour::Behaviour()
{
 ...
 app_leftdribblev_ = 0;
 app_rightdribblev_ = 0;

 robot_pos_  =  DPoint(0,0);
 robot_ori_  =  Angle(0.0);
 robot_vel_  =  DPoint(0,0);
 ball_pos_   =  DPoint(0,0);
 ball_vel_   =  DPoint(0,0);
...
Behaviour::move2Position(float pval, float dval, DPoint target, float maxvel)  // move to the target point by using PD control
{
    ...
    float _pos_e = robot_pos_.distance(target);
    DPoint relposoftarget =  target  - robot_pos_;
    // DPoint2f(target.x_ - m_worldmodel_->robot_pos_.x_, target.y_- m_worldmodel_->robot_pos_.y_);
    ...
    //m_worldmodel_->robot_ori_.radian_ = tar_theta;

    app_vx_ =  speed*cos(tar_theta - robot_ori_.radian_);  //
    app_vy_ =  speed*sin(tar_theta - robot_ori_.radian_);
...
Behaviour::move2target(float pval, float dval,DPoint target, DPoint realtarvel, float maxvel)
{
    ...
    app_vx_ =  speed*cos(tar_theta - robot_ori_.radian_) + compensation_vel.x_;  //
    app_vy_ =  speed*sin(tar_theta - robot_ori_.radian_) + compensation_vel.y_;
...
Behaviour::rotate2AbsOrienation(float pval, float dval, float orientation,float maxw)
{
    ...
    float theta_e = orientation-  robot_ori_.radian_;
