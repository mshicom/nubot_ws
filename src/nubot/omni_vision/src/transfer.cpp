...
Transfer::Transfer(std::string infopath,Omni_Image & _omni_img)
{
    omni_img_=&_omni_img;
...
vector<DPoint2i> Transfer::imagecoordunates(const vector<PPoint> & real_coor)
{
        size_t numtrans=real_coor.size();
        vector<DPoint2i> image_pts;
        image_pts.reserve(numtrans);
        for(size_t i=0; i<numtrans; i++)
          image_pts.push_back(imagecoordunates(real_coor[i]));
        return image_pts;
}

DPoint2i Transfer::imagecoordunates(const PPoint & real_coor)
{
  double image_distance=std::atan(real_coor.radius_/(100.0*para_a_))/para_b_;
  DPoint2i image_pt=DPoint2i(int(image_distance*cos(real_coor.angle_.radian_)),int(image_distance*sin(real_coor.angle_.radian_)));
  return (image_pt+DPoint2i(center_));
}

vector<DPoint2i> Transfer::imagecoordunates(const vector<DPoint> & world_coor,const DPoint & location ,const Angle & facing)
{
    vector<PPoint> real_pts=realcoordinates(world_coor,location,facing);
    return imagecoordunates(real_pts);

}

DPoint2i Transfer::imagecoordunates(const DPoint & world_coor,const DPoint & location ,const Angle & facing)
{
   PPoint real_pts=realcoordinates(world_coor,location,facing);
   return imagecoordunates(real_pts);
}
