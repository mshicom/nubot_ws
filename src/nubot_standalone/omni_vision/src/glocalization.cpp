...
void 
Globallocalization::rankErrorsforSamples(const std::vector<double> & _weights,const std::vector<PPoint> &  _white_pt,Angle & _angle,int _times)
{
	...
	//std::partial_sort(samples_.begin(), samples_.begin()+5,samples_.end(),[](location_err &loc1,location_err & loc2){ return (loc1.error<loc2.error);}); 
}
...
Globallocalization::process(const std::vector<double> & _weights,const std::vector<PPoint> &  _white_pt,DPoint & _robotloction, Angle & _angle)
{
	...
	rankErrorsforSamples(_weights,_white_pt,_angle,times);
