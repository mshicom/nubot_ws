...
Omni_Vision()
{
    ...
    imginfo_     = new Omni_Image("/home/nubot/omnivision/ROI.xml");
    tranfer_     = new Transfer("/home/nubot/omnivision/disofpic.txt",*imginfo_);
    scanpts_     = new ScanPoints(*imginfo_);
    whites_      = new Whites(*scanpts_,*tranfer_);
    optimise_    = new Optimise("/home/nubot/omnivision/errortable.txt","/home/nubot/omnivision/difftable.txt",*tranfer_);
    ...
    ball_finder_ = new BallFinder("/home/nubot/omnivision/CTable.dat",*tranfer_);
    ros::NodeHandle nh;
    image_transport::ImageTransport it(nh);
    img_sub_= it.subscribe("/camera/image_raw", 0, &Omni_Vision::imageCallback,this);
    ros::NodeHandle local_nh;
    motor_info_sub_ = local_nh.subscribe("/nubotdriver/odoinfo", 0, &Omni_Vision::odometryupdate,this);

    ros::NodeHandle node;
    ballinfo_pub_      = node.advertise<nubot_standalone::BallInfo>("/omnivision/ballinfo",0);
    robotinfo_pub_     = node.advertise<nubot_standalone::RobotInfo>("/omnivision/robotinfo",0);
    obstaclesinfo_pub_ = node.advertise<nubot_standalone::ObstaclesInfo>("/omnivision/obstcalesinfo",0);
    omin_vision_pub_   = node.advertise<nubot_standalone::OminiVisionInfo>("/omnivision/OminiVisionInfo",0);
}
~Omni_Vision()
{
...
void
odometryupdate(const nubot_standalone::VelCmd & _motorinfo_msg)
{
