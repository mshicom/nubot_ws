...
Localization::Localization(Optimise & _optimise,Odometry & _odometry)
{
	opti_     = &_optimise;
	odometry_ = &_odometry;
	variant_ang_=0.05;
...
void
Localization::update_odometry(DPoint & _delta_pos,Angle & _delta_ang,DPoint & _robot_pos,Angle & _robot_ang)
{
	_delta_pos=odometry_->get_world_locaton();
	_delta_ang=odometry_->get_delta_angle();
//  ROS_INFO("x: %f y: %f vx: %f  vy: %f angle: %f ",_delta_pos.x_,_delta_pos.y_,
//           odometry_->get_world_velocity().x_,odometry_->get_world_velocity().y_,_delta_ang.radian_);
	_robot_pos=_robot_pos+_delta_pos;
	_robot_ang=_robot_ang+_delta_ang;
	odometry_->clear(_robot_ang);
}
...
void
Localization::process(const std::vector<double> & _weights,const std::vector<PPoint> & _white_pt, DPoint & _robotloction, Angle & _angle)
{
	...
	DPoint2d delta_pos ;
    Angle    delta_ang;
	update_odometry(delta_pos,delta_ang,_robotloction,_angle);

    DPoint tempglobalpos   = _robotloction;
    Angle  tempglobalangle = _angle;

	double referror=0.0;
	static bool first_optimise=false;
	static int cycles_since_reset=0;
	if (numtrans>15)
		referror=opti_->process(_weights,_white_pt,tempglobalpos,tempglobalangle);
    if(numtrans>20)     //  if (internal_alternatives && lines.objectlist.size()>=10)
	{
		...
		for (size_t i=0; i<4; i++) 
		{
			...
			double err = opti_->process(_weights,_white_pt,trial_pos,trial_heading);
			if (err<min_error && sfac*err<referror)
			{
				min_index=i;
				min_error=err;
				tempglobalpos=trial_pos;
				tempglobalangle=trial_heading;
	...
	if (numtrans>15)
		opti_->analyse(_weights,_white_pt,ddxy, ddphi, tempglobalpos, tempglobalangle);
    ...
    if (numtrans>15)
		kalmanfilter(delta_pos,delta_ang,tempglobalpos,tempglobalangle,var_xy,var_phi,pos_odo,facing_odo);
    else
