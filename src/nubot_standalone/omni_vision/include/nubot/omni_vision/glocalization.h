...
public:
	struct location_err
	{
		...
		Angle  ang;
		double error;
		bool operator < (const location_err b)
		{
...
public:
	...
	void rankErrorsforSamples(const std::vector<double> & _weights,const std::vector<PPoint> &  _white_pt, Angle & _angle,int _times);
