...
Strategy::Strategy()
{
    ...
    robot_pos_  =  m_plan_.robot_pos_;
    robot_ori_  =  m_plan_.robot_ori_;
    robot_vel_  =  m_plan_.robot_vel_;
    ball_pos_   =  m_plan_.ball_pos_;
    ball_vel_   =  m_plan_.ball_vel_;
    dribble_state_ = false;
    game_ctrl_  =  CTRL_STOP;
    target_ = DPoint(0,0);
    target_orientation_ = 0;
    NeedEvaluat  = false;
    stucktime_ = 0;
    running = false;
}
...
void  Strategy::turn4Shoot()
{
        ...
        double Bodyfacing = robot_ori_.radian_;
        double theta2target =  thetaof2p(robot_pos_,kicktarget);
...
void Strategy::dynamicQuickShoot()
{
    ...
    double Bodyfacing  = robot_ori_.radian_;
    DPoint robotglobalpos = robot_pos_;
...
{
    DPoint tmpball = ball_pos_;
    DPoint tmp;
    tmp = PointOfLine(tmpball,DPoint(-900,0),indist_);

    if (target_orientation_ != 0)
        tmp = RotOfPoint(tmp,tmpball,target_orientation_,true);
    return tmp;
}
...
Strategy::activeCatchBall()
{
    ...
    if(ball_info_state_ == CannotSeeBall)
    {
        return;
    }

    if(IsLocationInField(robot_pos_))
    {
       m_plan_.catchBall();
    }
    else
...
Strategy::activeCatchBall()
{
        ...
        DPoint MyGlobalPos   = robot_pos_;

        if(BallGlobalpos.distance(MyGlobalPos)<VISIONSCALE)
            BallGlobalVel=ball_vel_;
        else
        ...
        DPoint catchspeed=vrel2global(DPoint(150,0),robot_ori_);//
...
{
    DPoint robotglobalpos   =  robot_pos_;
    float  robotorientation =  robot_ori_.radian_;

    bool   is_robot_stuck   =  is_robot_stuck_;
    int    ball_info_state  =  ball_info_state_;
    bool   dribble_state     =  dribble_state_;
    int    currentactiverobotnum = active_robot_num_;
...
Strategy::checkShootDirection(float direction,float swidth,float lwidth,float len)
{
    ...
    {
        if(IsInRectangle(trap[0],trap[1],trap[2],trap[3],obs_pos_[k]))
        {
           return false;
        }
    }
...
Strategy::evaluateKick(DPoint &target,float & leftdelta,float &rightdelta)
{
    ...
    //						&& (FIELDLENGTH/2-abs(m_WorldModel->ball.GlobalPos.X))<(abs(m_WorldModel->ball.GlobalPos.Y)-100)/1.5)
     ...
     float   dis2oppgoal =  robot_pos_.distance(opp_goal_pos_);
     float   direction   =  thetaof2p(robot_pos_,opp_goal_pos_);
     DPoint  ballglobalpos = ball_pos_;
    ...
    for (int j=1;j<GoalPointNum-1;j++)
    {
        for(int i=0;i<OBSRECORDNUM;i++)
        {
            ...
            tempIsInParallelogram  = IsInRectangle(DPoint(ballGlobalpos.x_,ballGlobalpos.y_-20.0),GoalPoint[j-1],GoalPoint[j+1],
                                                   DPoint(ballGlobalpos.x_,ballGlobalpos.y_+20.0),obs_pos_[i]);
...
Strategy::runGame()
{
    //dribble_state_ = checkRobotDrible();
     ...
     case CTRL_ZONEDEF:    // £¡
          ...
          rel_target =  pglobal2rel(robot_pos_,robot_ori_.radian_,target);
          rel_target_ori  = target_orientation - robot_ori_.radian_;
     ...
     case CTRL_DEFENCE:
        ...
        // if(ball_info_state_ != NOTSEEBALL )
        //     target_orientation =  thetaof2p(robot_pos_,ball_pos_);
        // else
         target_orientation =  thetaof2p(robot_pos_,ball_pos_);
     ...
     case CTRL_CATCH:    //  µ÷ÓÃ×¥Çòº¯Êý
         ...
         {
            ballpos_record_ = ball_pos_;
            distance2ball_record_ = robot_pos_.distance(ballpos_record_);
            game_begin_ = true;
         }
         if(distance2ball_record_< 340.0 && distance2ball_record_>= 0.0 && ball_info_state_ != CannotSeeBall)
         {
             distance2recordball = ballpos_record_.distance(ball_pos_);
     ...
     case CTRL_CATCH_MOVE:
        ...
        // if(ball_info_state_ != NOTSEEBALL )
         target_orientation =  thetaof2p(robot_pos_,ball_pos_);
         //    else
         //    target_orientation =  thetaof2p(robot_pos_,m_WorldModel->MsgFromCoach.FuseBall);
         m_plan_.m_behaviour_.move2Position(2.75,1.5,target,MAXVEL);
